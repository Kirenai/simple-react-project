...
const TaskForm = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (id)
      await axios.put(`http://localhost:8080/api/task/update/${id}`, task);
  ...
  const getTask = async (id: string) => {
    ...
    const { title, author, description } = response.data.taskData;
    setTask({
      taskData: {
        title,
        author,
  ...
  const { title, author, description } = task.taskData;

  return (
    <div className="container mx-auto">
      <div className="flex justify-center items-start h-full p-4">
        <form
          className="p-8 rounded-xl bg-gray-600 shadow-xl"
          onSubmit={handleSubmit}
        >
          <div className="py-2">
            <label
              className="w-full block text-xl text-gray-100"
              htmlFor="title"
            >
              Title
            </label>
